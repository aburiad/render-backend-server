/**
 * Concurrent Load Test — 20 Users
 * Uses test-question.jpg from project root
 * 
 * Usage:  node test-20users-concurrent.cjs
 */
...
const RENDER_URL = 'https://render-backend-server-irsv.onrender.com'

const TOTAL_USERS = 20
const EMAIL_PREFIX = 'testuser'
const EMAIL_DOMAIN = 'proshnoshala.com'
const PASSWORD = 'test123456'
const LOGIN_DELAY_MS = 1200   // Supabase rate limit protection
...
async function scan(token, userNum, imgBase64) {
  ...
  try {
    const res = await fetch(`${RENDER_URL}/api/ai/scan`, {
      method: 'POST',
      headers: {
...
;(async () => {
  console.log(`\n${'═'.repeat(60)}`)
  console.log(`🚀 CONCURRENT LOAD TEST — 20 USERS × 1 IMAGE SCAN`)
  ...
  if (!fs.existsSync(IMAGE_PATH)) {
    ...
    console.error('   Place test-question.jpg in the project root.')
    process.exit(1)
  }
  ...
  if (tokens.length === 0) {
    ...
    console.error('   Make sure test users exist. Run: node testapp/create_dummy_users.cjs')
    return
  }

  // ── Phase 2: Concurrent scans ──
  console.log(`📋 Phase 2: Firing ${tokens.length} concurrent scan requests...\n`)
  ...
  const p50 = results.sort((a, b) => a.time - b.time)[Math.floor(results.length * 0.5)]?.time || 0
  const p90 = results.sort((a, b) => a.time - b.time)[Math.floor(results.length * 0.9)]?.time || 0
  const maxTime = Math.max(...results.map(r => r.time))
  const minTime = Math.min(...results.map(r => r.time))

  console.log(`\n${'═'.repeat(60)}`)
  console.log(`📊 SUMMARY — ${tokens.length} CONCURRENT USERS`)
  console.log(`${'═'.repeat(60)}`)
