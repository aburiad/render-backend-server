...
function PaperItem({ paper, onLongPress, onDelete, onDuplicate, deleting, duplicating, omrPickerMode = false }) {
  ...
  return (
    <>
      ...
      {/* Action menu — BottomSheet slides up from bottom (mobile-friendly) */}
...
export default function PapersList() {
  const papers = usePaperListStore((s) => s.list)
  const listFetchedAt = usePaperListStore((s) => s.listFetchedAt)
  const loadList = usePaperListStore((s) => s.loadList)
  const revalidateList = usePaperListStore((s) => s.revalidateList)
  const upsertPaper = usePaperListStore((s) => s.upsertPaper)
  const removePaperFromCache = usePaperListStore((s) => s.removePaper)

  const [loading, setLoading] = useState(papers.length === 0)
  ...
  useEffect(() => {
    ...
    if (papers.length > 0) {
      setLoading(false)
      const stale = !listFetchedAt || Date.now() - listFetchedAt > 5 * 60 * 1000
      if (stale) revalidateList()
    } else {
      loadList()
        .catch(() => {
          if (!cancelled) toast.error('প্রশ্নপত্র লোড করতে ব্যর্থ')
        })
        .finally(() => {
  ...
  async function handleRefresh() {
    await loadList({ force: true, silent: true })
  }
    ...
    >
      ...
      <div className="hidden lg:flex" style={{ alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          <RefreshButton onRefresh={handleRefresh} title="তালিকা রিফ্রেশ" />
      ...
      {/* Mobile refresh row (lg-hidden) */}
      <div className="flex lg:hidden" style={{ justifyContent: 'flex-end' }}>
        <RefreshButton onRefresh={handleRefresh} title="তালিকা রিফ্রেশ" size={32} />
      </div>
