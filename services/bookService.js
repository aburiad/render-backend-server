...
const bookService = {
  ...
  async getExistingQuestions(classNum, subject, selections, filters = {}) {
    console.log('[getExistingQuestions] selections:', JSON.stringify(selections?.map(s => ({ ch: s.chapterId, count: s.count, subs: s.subchapterIds }))))
    console.log('[getExistingQuestions] filters:', JSON.stringify(filters))
    const results = []
