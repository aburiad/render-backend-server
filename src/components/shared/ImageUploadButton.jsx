...
import { cropAndEnhance, enhanceDiagram } from '@/lib/enhanceImage'
...
/**
 * Reusable image upload with crop + H/W controls + annotation.
...
export default function ImageUploadButton({
  ...
  const handleFile = (file) => {
    ...
    reader.onloadend = () => {
      console.log('[upload] File read complete, data URL length:', reader.result?.length)
  ...
  const confirmCrop = async () => {
    console.log('[upload] confirmCrop — imgRef:', !!imgRef.current, 'completedCrop:', completedCrop)
    if (!imgRef.current || !completedCrop || completedCrop.width === 0) {
      console.log('[upload] No crop region, using full image...')
      try {
        const enhanced = await enhanceDiagram(rawImage)
        console.log('[upload] Enhanced OK, length:', enhanced?.length)
        onChange(enhanced)
      } catch (e) { console.error('[upload] Enhance failed:', e); onChange(rawImage) }
      setRawImage(null)
      return
    }
    setEnhancing(true)
    try {
      ...
      console.log('[upload] Cropped OK, length:', cropped?.length)
      onChange(cropped)
      toast.success('ডায়াগ্রাম ক্রপ হয়েছে!')
    } catch (e) {
      console.error('[upload] Crop failed:', e)
      toast.error('ক্রপ করতে সমস্যা হয়েছে')
      try {
        const enhanced = await enhanceDiagram(rawImage)
        onChange(enhanced)
      } catch (e2) { console.error('[upload] Fallback enhance failed:', e2); onChange(rawImage) }
    } finally {
      setEnhancing(false)
      setRawImage(null)
    }
  ...
  const useFullImage = async () => {
    console.log('[upload] useFullImage')
    setEnhancing(true)
    try {
      const enhanced = await enhanceDiagram(rawImage)
      console.log('[upload] Full image enhanced OK, length:', enhanced?.length)
      onChange(enhanced)
    } catch (e) { console.error('[upload] Full enhance failed:', e); onChange(rawImage) }
        ...
        <div className="flex flex-col gap-1.5">
          <div className="relative group inline-block">
            ...
            <div className="absolute top-1 right-1 flex gap-1 md:opacity-0 md:group-hover:opacity-100 transition-opacity">
              ...
              <button type="button"
                onClick={() => setAnnotateImage(value)}
                className="px-1.5 py-1 bg-white/90 text-purple-600 rounded-lg text-[10px] font-bold shadow-sm hover:bg-purple-50"
                title="✏️ ছবিতে লিখুন/আঁকুন">
                ✏️
              </button>
          ...
          <div className="flex items-center gap-1.5 flex-wrap">
            <button type="button"
              onClick={() => { console.log('[upload] ✏️ annotate clicked, value:', !!value); setAnnotateImage(value) }}
              className="inline-flex items-center gap-1 px-2 py-1 bg-purple-50 text-purple-700 rounded-lg text-[11px] font-bold hover:bg-purple-100 transition-colors border border-purple-200"
              title="✏️ ছবিতে লিখুন/আঁকুন">
              ✏️ আঁকুন
            </button>
            <button type="button"
              ...
              disabled={enhancing}
              className="inline-flex items-center gap-1 px-2 py-1 bg-blue-50 text-blue-700 rounded-lg text-[11px] font-bold hover:bg-blue-100 transition-colors border border-blue-200"
              title="আবার উন্নত করুন">
              {enhancing ? '...' : '✨ উন্নত'}
            </button>
            <button type="button"
              onClick={() => { setRawImage(value); setCrop({ unit: '%', width: 90, x: 5, y: 5, height: 90 }); setCompletedCrop(null) }}
              className="inline-flex items-center gap-1 px-2 py-1 bg-emerald-50 text-emerald-700 rounded-lg text-[11px] font-bold hover:bg-emerald-100 transition-colors border border-emerald-200"
              title="✂️ আবার ক্রপ করুন">
              ✂️ ক্রপ
            </button>
            <button type="button" onClick={() => onChange(null)}
              className="inline-flex items-center gap-1 px-2 py-1 bg-red-50 text-red-600 rounded-lg text-[11px] font-bold hover:bg-red-100 transition-colors border border-red-200"
              title="ছবি মুছুন">
              ✕ মুছুন
            </button>
          </div>
