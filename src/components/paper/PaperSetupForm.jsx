...
import HeaderPresetSelector from '@/components/paper/HeaderPresetSelector'
      ...
      <AnimatePresence>
        {expanded && (
          ...
          >
            ...
            <InputGroup label="হেডার প্রিসেট নির্বাচন">
              <HeaderPresetSelector
                value={paper?.header_preset || ''}
                onChange={(value) => handleChange('header_preset', value)}
              />
            </InputGroup>
            ...
            <InputGroup label="প্রতিষ্ঠানের লোগো">
              <div className="flex items-center gap-3 sm:gap-4">
                <label
                  className="w-12 h-12 sm:w-[60px] sm:h-[60px] rounded-xl sm:rounded-2xl flex items-center justify-center relative flex-shrink-0 cursor-pointer"
                  style={{
                ...
                >
                  {paper?.logo_url ? (
                    <>
                      <img
                        src={paper.logo_url} alt="Logo"
                        style={{ width: '100%', height: '100%', objectFit: 'contain', borderRadius: 14 }}
                      />
                      <button
                        type="button"
                        onClick={(e) => { e.preventDefault(); e.stopPropagation(); handleChange('logo_url', null) }}
                        style={{
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      if (!file) return
                      // Resize to max 200x200 and convert to data URL for storage
                ...
                <div className="flex flex-col gap-1.5">
                  <p className="text-[10px] sm:text-[11px] text-slate-400 font-medium leading-snug">
                    ক্লিক করে লোগো আপলোড করুন (PNG/JPG)
                  </p>
                  {/* Logo alignment */}
                  {paper?.logo_url && (
                    <div className="flex items-center gap-1">
                      <span className="text-[9px] font-bold text-slate-400 mr-1">অ্যালাইন:</span>
                      {[
                        { value: 'left', icon: '⬅️', label: 'বামে' },
                        { value: 'center', icon: '⬆️', label: 'মাঝে' },
                        { value: 'right', icon: '➡️', label: 'ডানে' },
                      ].map((opt) => {
                        const active = (paper?.print_settings?.logoAlignment || 'left') === opt.value
                        return (
                          <button
                            key={opt.value}
                            type="button"
                            onClick={() => handlePrintSettingChange('logoAlignment', opt.value)}
